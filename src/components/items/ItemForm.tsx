...
import { AlertTriangle, Sparkles, Zap, Key, Save, X, Plus, Tag, DollarSign, FileText, Camera, Image as ImageIcon } from 'lucide-react';
import { AIDetectionService } from '../../services/aiDetection';
import { StorageService } from '../../services/storage';
import { CollectibleData, DetectionResult, FolderType, ItemCondition } from '../../types';
...
export const ItemForm: React.FC<ItemFormProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const imageUrls = item
        ? [item.primaryImage, ...item.additionalImages].filter(Boolean)
        : [];
      ...
      // Set default tab based on whether it's a new item or editing
      setActiveTab(item ? 'manual' : 'ai');
  ...
  const handleImagesChange = async (newImages: ImageFile[]) => {
    ...
    // Always run AI detection on new images in AI tab
    if (activeTab === 'ai' && newImages.length > 0) {
  ...
  const handleAIDetectionForImage = async (imageFile: ImageFile) => {
    ...
    const {
      ...
      remaining,
      isUsingCustomKey: usingCustom,
    } = await aiService.canUseDetection();
    ...
    try {
      ...
      const { remaining: newRemaining, isUsingCustomKey: newUsingCustom } =
        await aiService.canUseDetection();
  ...
  const handleCameraCaptureWithAI = async (imageBlob: Blob) => {
    ...
    // Then run AI detection on it (always in AI tab)
    const imageUrl = URL.createObjectURL(imageBlob);
    await handleAIDetection(imageBlob, imageUrl);
  };

  const handleDetectionResultsSave = async (
  ...
  const handleApiKeySet = async (newApiKey: string) => {
    ...
    try {
      ...
      // Update usage info
      const { remaining, isUsingCustomKey: usingCustom } =
        await aiService.canUseDetection();
      setRemainingDetections(remaining);
      setIsUsingCustomKey(usingCustom);
    } catch (error) {
  ...
  const processImagesForSave = async (
    ...
    if (imageFiles[0]) {
      try {
        ...
        if (
          imageFiles[0].url.startsWith("data:") ||
          imageFiles[0].url.startsWith("blob:")
        ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasImagesChanged() && images.some((img) => img.isEdited)) {
      if (
        !window.confirm(
          "You have edited images. Do you want to save these changes?",
        )
      ) {
        return;
      }
  ...
  const handleAddTag = () => {
    ...
    const validatedTags = validateAndFilterTags(
      [...formData.tags, tag],
      formData.type,
    );
    ...
    if (validatedTags.length === formData.tags.length) {
      ...
      if (formData.type && tag === formData.type.toLowerCase()) {
        setErrors({
          tag: `Tag "${tag}" matches the item type and will be automatically excluded.`,
        });
    ...
    if (
      formData.tags.some((existingTag) => existingTag.toLowerCase() === tag)
    ) {
  ...
  const handleClose = () => {
    ...
    console.log(
      "Attempting to close form. Has unsaved changes:",
      unsavedChanges,
    );
  ...
  const shouldShowFreeLimitWarning =
              ...
              <Card
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-6 h-6 rounded-pixel flex items-center justify-center ${
                        isUsingCustomKey
                          ? "bg-retro-success"
                          : isDevUnlimited
                            ? "bg-retro-primary"
                ...
                <div className="space-y-pixel-2">
                  ...
                  {isDetecting && (
                    <Card
                      variant="outlined"
                      padding="md"
                      className="bg-retro-accent bg-opacity-10 border-retro-accent">
                      <div className="flex items-center gap-2">
                        <LoadingSpinner
                          size="sm"
                          variant="accent"
                        />
                        <div>
                          <p className="font-pixel text-retro-accent text-sm">
                  ...
                  {shouldShowFreeLimitWarning && (
                    <Card
                      variant="outlined"
                      className="border-retro-error">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <AlertTriangle className="w-4 h-4 text-retro-error" />
                          <div>
                            <h4 className="font-pixel text-retro-error text-sm">
                              Free Limit Reached
                            </h4>
                            <p className="text-retro-error font-pixel-sans text-xs">
                              Add your own Gemini API key to continue using AI
                              detection.
                            </p>
                          </div>
                        </div>
                ...
                <div className="space-y-pixel">
                  ...
                  {formData.type && (
                    <div className="text-xs text-retro-accent-light font-pixel-sans">
                      💡 Tags that match the item type ("{formData.type}") will
                      be automatically excluded to avoid duplicates.
                    </div>
                  )}
