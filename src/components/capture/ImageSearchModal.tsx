...
import { Search, Download, X, ExternalLink, Image as ImageIcon, Check, Loader } from 'lucide-react';
...
export const ImageSearchModal: React.FC<ImageSearchModalProps> = ({
  ...
  const handleSearch = async (query?: string) => {
    ...
    try {
      ...
      if (results.length === 0) {
        setError('No images found. Try adjusting your search terms.');
      }
    } catch (error) {
      ...
      <div className="space-y-pixel-2">
        ...
        <div className="flex items-center gap-2 p-3 bg-retro-bg-tertiary border border-retro-accent rounded-pixel">
          ...
          <div>
            ...
            <p className="text-retro-accent-light font-pixel-sans text-xs">
              Find high-quality images that match your item instead of using your photo
            </p>
          </div>
        </div>
        ...
        {!isSearching && searchResults.length === 0 && searchQuery && !error && (
          <div className="text-center py-pixel-4">
            ...
            <p className="text-retro-accent-light font-pixel-sans text-sm">
              Try different search terms or use your own photo instead
            </p>
          </div>
        )}
        ...
        <Card variant="outlined" padding="md" className="bg-retro-bg-tertiary">
          <h4 className="font-pixel text-retro-accent text-sm mb-2">How it works:</h4>
          <ul className="space-y-1 text-xs font-pixel-sans text-retro-accent-light">
            <li>• Search results come from high-quality stock photo sources</li>
            <li>• Click any image to download and use it for your item</li>
            <li>• The selected image will replace your uploaded photo</li>
            <li>• You can still crop and edit the downloaded image</li>
          </ul>
        </Card>
